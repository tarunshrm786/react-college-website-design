...
const Mentor = () => {
  ...
  useEffect(() => {
    const fetchMentors = async () => {
      try {
        ...
        const data = response.data.map((mentor) => ({
          ...mentor,
          base64Image: mentor.image?.data?.data
            ? `data:${mentor.image.contentType};base64,${arrayBufferToBase64(mentor.image.data.data)}`
  ...
  const handleFileChange = (event) => {
    ...
    if (file) {
      // Check for file size greater than 1 MB (1,048,576 bytes)
      if (file.size > 1048576) {
        setSnackbarMessage('Image size must be less than 1 MB.');
        setSnackbarOpen(true);
        setSelectedFile(null);
        return;
      }
  ...
  const handleAddMember = async () => {
    ...
    try {
      const response = await axios.post('https://nad-api-tarunshrm768gmailcoms-projects.vercel.app/api/mentors', formData);
  ...
  const handleDeleteMember = async () => {
    try {
      const response = await axios.delete(`https://nad-api-tarunshrm768gmailcoms-projects.vercel.app/api/mentors/${deleteId}`);
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      ...
      <Box sx={{ display: 'flex', flexGrow: 1 }}>
        ...
        <Box sx={{ flexGrow: 1, p: 4, marginTop: '55px' }}>
          ...
          <Button variant="contained" onClick={handleAddMember} sx={{ bgcolor: 'black', color: 'white' }}>
            Add Mentor
          </Button>
          ...
          <Box sx={{ mt: 4 }}>
            {teamMembers.map((member) => (
              <Card key={member._id} sx={{ mb: 2, display: 'flex', alignItems: 'center' }}>
                ...
                <Button sx={{ color: 'red' }} onClick={() => handleOpenDeleteModal(member._id)}>
                  <DeleteIcon />
                </Button>
              </Card>
            ))}
