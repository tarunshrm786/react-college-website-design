...
const Affiliation = () => {
  ...
  // Fetch affiliation logos from the API
  useEffect(() => {
    const fetchLogos = async () => {
      try {
        const response = await fetch(
          'https://nad-api-tarunshrm768gmailcoms-projects.vercel.app/api/affiliation'
        );
        ...
        console.log('Affiliation API Response:', data);
         ...
         const images = data.map((affiliation) => ({
          src: `data:${affiliation.images[0].imageData}`, // Assuming imageData contains the base64-encoded image
          alt: affiliation._id || 'Affiliation Image',
        }));

        console.log('Processed Affiliation Images:', images);
      ...
      <Slider {...settings}>
        {logos.map((logo, index) => (
          ...
          >
            ...
            >
              <img
                src={logo.src} // Base64 image data directly in src
                alt={logo.alt} // Alt text
                loading="lazy"
                style={{
                  maxHeight: '100px', // Fixed height for all logos
                  width: '100%', // Adjust to fit the container
